...
export class DocumentsAgent implements BaseAgent {
  metadata(): AgentMetadata {
    return {
      name: 'DocumentsAgent',
      domain: 'Document Audit & Verification',
      description: 'Handles document verifications, compliance checks, and outstanding required files summaries.',
      supportedCapabilities: ['document_audit', 'compliance_verification', 'missing_backlogs'],
      toolsOwned: ['get_missing_documents', 'get_registration_conversion_summary']
    };
  }

  canHandle(query: string): boolean {
    const q = query.toLowerCase();
    return q.includes('document') || q.includes('doc') || q.includes('upload') || q.includes('verification') || q.includes('certificate') || q.includes('photo');
  }
...
Do not write any markdown formatting, only valid JSON.`,
      ...
      return {
        ...
        reasoning: parsedResult.reasoning || "Executed document audit query successfully.",
