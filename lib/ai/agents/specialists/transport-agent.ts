...
export class TransportAgent implements BaseAgent {
  metadata(): AgentMetadata {
    return {
      name: 'TransportAgent',
      domain: 'Student Transportation',
      description: 'Handles bus routing assignment, stops manifests, and transport capacity logistics.',
      supportedCapabilities: ['transport_allocations', 'route_manifest', 'bus_capacities'],
      toolsOwned: ['get_transport_summary', 'get_route_manifest']
    };
  }

  canHandle(query: string): boolean {
    const q = query.toLowerCase();
    return q.includes('transport') || q.includes('route') || q.includes('stop') || q.includes('bus') || q.includes('driver') || q.includes('vehicle') || q.includes('van');
  }
...
Do not write any markdown formatting, only valid JSON.`,
      ...
      return {
        ...
        reasoning: parsedResult.reasoning || "Executed transport query successfully.",
